...
final class BM25Scorer extends Scorer {
	 ...
	 *            <code>Term</code>.
	 * @param reader 
	 * 			  The indexReader used to get the documents for further scoring
	 */
	BM25Scorer(Weight weight, TermDocs td, Similarity similarity,
			byte[] norms, float idfValue, float avg, IndexReader reader) {
		super(similarity, weight);

		this.termDocs = td;
		this.norms = norms;
		this.weightValue = weight.getValue();		// this is just the idf of the term
		this.idf = idfValue;
		this.avgLength = avg;
		this.reader = reader;
